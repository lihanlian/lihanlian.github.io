...
- ### Example: Cartpole Balancing (<i class="fa-brands fa-github"></i> [cartpole-dqn-ddpg-ppo](https://github.com/lihanlian/cartpole-dqn-ddpg-ppo))
  ...
  class PPOValueNetwork(nn.Module):
      ...
      def forward(self, x: torch.Tensor):
          ...
          return self.net(x)    
  ```
  Note that policy network only outputs a mean $$\mu(s)$$, and here we build a Gaussian via a scheduled standard deviation $$\sigma$$. Many continuous-control PPO implementations use **fix** or **decay** standard deviation over training, rather than learning it. This reduces the number of trainable parameters and can make exploration more predictable, as long as the tuned schedule results in enough exploration (large $$\sigma$$) in the early training stage, and later anneals toward more deterministic policies.
  ...
  - Make Data
    ...
    This gathers rollouts into minibatches and computes, for each time step, the TD-target, temporal-difference error ($$\delta$$), and **generalized advantage estimate (GAE)**. It returns a list of tuples: `(obs, action, log_prob, target, advantage)` ready for PPO updates. The reversed loop implements **backward-accumulation of GAE**.
