...
- **Hamiltonian Mechanics**
  ...
  Hamiltonian mechanics is a reformulation of classical mechanics. It provides powerful tools and a different perspective compared to the Lagrangian approach. Here is a more detailed framework:

  1. **Hamiltonian $$H $$:**

    - **Definition:** The Hamiltonian $$H $$ is defined as:
      $$
      H(q, p, t) = p \cdot \dot{q} - L(q, \dot{q}, t)
      $$
    - **Interpretation:** It often represents the total energy of the system (kinetic + potential energy) but is formulated in terms of coordinates $$q $$ and canonical momenta $$p $$.

  2. **Canonical Coordinates:**

    - **Position ($$q $$)**: Generalized coordinates describing the configuration of the system.
    - **Momentum ($$p $$)**: Canonical momenta conjugate to $$q $$, defined as:
      $$
  ...
  3. **Hamilton's Equations of Motion:**
    ...
    These are first-order differential equations that describe the time evolution of the canonical coordinates:
    $$
 ...
 - **Principle of Least Action (Hamilton's Principl)**
    ...
    Hamilton's Principle, also known as the principle of least action, states that the true path taken by a physical system between two points in time is the one that minimizes the action $$S $$. The action $$S $$ is defined as the integral of the Lagrangian $$L $$ over time:

    $$ S = \int_{t_1}^{t_2} L(q, \dot{q}, t) \, dt $$

    Here:
    - $$q(t) $$ are the generalized coordinates.
    - $$\dot{q}(t) $$ are the velocities.
    - $$L(q, \dot{q}, t) $$ is the Lagrangian, typically the difference between kinetic and potential energy.

    According to Hamilton's Principle, the actual path taken by the system makes the action $$S $$ stationary (usually a minimum). This leads to the Euler-Lagrange equations:
