...
In this case, \\(\delta q\\) can not be arbitrary because constraint needs to be satified. Since \\(\delta f(\delta q, t) = 0\\), we can mulitply it with Lagrange multipliers \\(\lambda\\) and add it into the integration but still keep the result unchanged. 
...
Since the control input directly impact on the robot's velocity, the objective functional and its corresponding Langrangina can be expressed as:
...
plt.subplot(1, 2, 1)
plt.plot(t, x_numeric, label='Numerical x(t)')
plt.plot(t, x_analytic, 'r--', label='Analytic x(t)')
...
plt.subplot(1, 2, 2)
plt.plot(t, u_numeric, label='Numerical u(t)', color='blue')
plt.plot(t, u_analytic, 'r--', label='Analytic u(t)')
