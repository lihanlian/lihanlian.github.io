...
$$ J = \frac{1}{2} \int_{0}^{T} (x(t)^TQx(t) + u(t)^TRu(t)) + \frac{1}{2} x(t)^T H x(t)\, dt $$
...
$$ \frac{\partial H}{\partial u} = Ru + B^T\lambda = 0, \quad  u^* = -R^{-1}B^T\lambda$$
...
Now suppose the costate $$\lambda$$ is linearly dependent on state $$x$$, and its time derivative can be expressed as follows:
...
$$
\dot{\mathbf{\lambda}}^*(t) = \mathbf{P}(t) \dot{\mathbf{x}}^*(t) + \mathbf{P}(t) \mathbf{x}^*(t)
$$

We have $$2n$$ first order differential equations, so we need $$2n$$ boundary conditions to solve the problem. Given initial condition of $$x_0$$, we have $$n$$ boundary conditions at the initial time of state equation. Recall we are dealing with the type of free final state fixed final time problem. Equipped with the tool we just derived from previous section, the other $$n$$ boundary conditions for this type of problem come from the final time of costate equations $$\lambda$$
<!-- 
...
$$ \dot{\lambda} = \dot{P}x + P(Ax - BR^{-1}B^{T}Px) = -Qx - A^TPx $$
