...
## Bellman's Principle of Optimality
 ...
 - **Continuous time (Derivation of HJB equation)**
    ...
    Define the <span style="color:red">minimum cost-to-go</span> function and take totoal derivatives:
...
Consider the problem that is to minimize the infinite-horizon quadratic cost function:
...
$$
\frac{\partial V(x)}{\partial t} + \min_u \left[ \frac{\partial V(x)}{\partial x} \cdot \dot{x} + x^T Q x + u^T R u \right] = 0
$$

For an infinite-horizon problem, $$ \frac{\partial V(x)}{\partial t} = 0 $$, so the HJB equation simplifies to:

$$
\min_u \left[ \frac{\partial V(x)}{\partial x} \cdot (A x + B u) + x^T Q x + u^T R u \right] = 0
$$

### Step 2: Value Function Assumption

Assume the value function V(x) is quadratic in the state $$x$$:

$$ V(x) = x^T P x $$

where $$ P $$ is a symmetric positive semi-definite matrix to be determined.

### Step 3: Compute the Gradients

The gradient of $$ V(x) $$ with respect to the state $$x $$ is:

$$ \frac{\partial V(x)}{\partial x} = 2x^T P $$

So the HJB equation becomes:

$$ \min_u \left[ 2x^T P (A x + B u) + x^T Q x + u^T R u \right] = 0 $$

### Step 4: Minimizing with Respect to u $$

To find the optimal control $$u^*$$, differentiate the expression inside the minimization with respect to $$ u $$ and set the derivative to zero:
...
### Step 5: Substitute u^* Back into the HJB Equation
...
### Step 6: Algebraic Riccati Equation (ARE)
