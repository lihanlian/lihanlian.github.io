...
$$
\frac{\partial V(x,t_{\infty})}{\partial t} + \min_u \left[ \left(\frac{\partial V(x,t_{\infty})}{\partial x} \right)^T \cdot \dot{x} + x^T Q x +  u^T R u \right] = 0
$$
...
$$
\min_u \left[ \left(\frac{\partial V(x,t_{\infty})}{\partial x} \right)^T \cdot (A x + B u) + x^T Q x + u^T R u \right] = 0
$$
...
$$ \frac{\partial V(x,t_{\infty})}{\partial x} = 2Px $$
...
$$ \frac{\partial}{\partial u} \left( x^T P B u + u^T R u \right) = 0 $$
