...
## Bellman's Principle of Optimality
 ...
 - **Continuous time (Derivation of HJB equation)**
    ...
    Given the initial time $$t_0$$, the optimization is formulated as follows:
    ...
    $$
    s.t. \quad \frac{d}{dt} \mathbf{x} = \mathbf{f}(\mathbf{x}(t), \mathbf{u}(t), t) t \in [t_0, t_{\infty}]
    $$

    First, define the <span style="color:red">minimum cost-to-go</span> function (also called as **Bellman value function**), since it is under the condition of optimal control $$u^*$$, both state and control input trajectories are determined. Thus, it is only a function of initial state $$x_0$$ and final time $$t_f$$.
