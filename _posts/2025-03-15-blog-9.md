...
- ### Tanh Normalization
  ...
  **2. Entropy under a transform (an expectation)**
  ...
  - For **metrics**, people often report

    $$
    \text{entropy} = -\,\text{log\_prob.mean()}
    $$
    for readability.
...
- **Critic Updates (with torch.no_grad and detach)**  
  ...
  (You will see the same idea in the actor update: we often detach $$\alpha$$ to
  avoid cross-talk from the actor loss into the temperature learner.)

  ---

  2) **What does `target_V` represent?**
  ...
  - `target_Q` is the **one-step bootstrap target** $$y$$ for the critic MSE:
    reward plus discounted `target_V`, masked by $$(1 - \text{terminated\_flag})$$.
  - The **double Q** term ($$\min$$ of two target critics) reduces positive
    bias / overestimation.

  So yes, it uses a single action sample per next state to approximate the
  expectation inside $$V(s')$$.

  ---

  3) **Should we use `with torch.no_grad()` when computing targets?**

  Yes, that is best practice. When you build the bootstrapped target $$y$$, you do
