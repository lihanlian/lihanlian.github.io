...
- ### 1.2 Metric:
  ...
  - ### Dice Coefficient:
    ...
    Here, $$\lvert P \cap G \rvert$$ is the number of pixels that are correctly predicted as foreground, $$\lvert P \rvert$$ is the total number of predicted foreground pixels, and $$\lvert G \rvert$$ is the total number of ground-truth foreground pixels. The factor of $$2$$ gives more weight to the overlapping region.
  ...
  - ### IoU:
    ...
    In practice, both metrics are useful for evaluating segmentation performance. Dice is often used as both an evaluation metric and a training loss, while IoU is commonly used to report the final segmentation quality. For this project, these metrics provide a direct way to measure how well the predicted pet mask matches the ground-truth mask.
