...
- ### 1.2 Metric:
  ...
  - ### Dice Coefficient:
    ...
    <span style="color:red"> Here, $$\lvert P \cap G \rvert$$ is the number of pixels that are correctly predicted as foreground, $$\lvert P \rvert$$ is the total number of predicted foreground pixels, and $$\lvert G \rvert$$ is the total number of ground-truth foreground pixels </span>. The factor of $$2$$ gives more weight to the overlapping region.
  ...
  - ### Implementation Details (jax.vmap)
    ...
    The following code snippet is used for loss calculation (metric.py):
